...
import pandas as pd
from web_app.DataGen import UPDATE_INTERVAL_SECONDS
...
# def get_avg_metric(cursor: Cursor, timestamps_by_sensor: list[tuple], col_n, selected_unit: str,
#                    selected_unit_modifier: str, metric_title: str, delta=None) -> None:
#     query = f'SELECT {selected_unit} FROM weather_data WHERE sensor_id = ? AND timestamp = ?'
#     results = []
...
#
#
# def get_hist_section(conn: Connection, start_date_time: datetime, end_date_time: datetime, just_id_choices: list[int],
...
#         query = (f"SELECT sensor_id, timestamp, {measured_unit} FROM weather_data "
...
#     df = df[(df['timestamp'] >= start_date_time) & (df['timestamp'] <= end_date_time)]
#
#     # Format the table
#     df['locale'] = df['sensor_id'].map(lambda x: id_to_locale_map[x])
#     pivot_df = df.pivot(index="timestamp", columns="locale", values=measured_unit)
#
#     # Create line chart
...
#             """)
#
#     timestamps_by_sensor = cursor.fetchall()
#
#     # Average Temperature
#     get_avg_metric(
#         cursor, timestamps_by_sensor, col1, selected_units["Temperature"], selected_unit_modifiers["Temperature"],
#         "Avg Temperature"
#     )
#
#     # Average Wind Speed
#     get_avg_metric(
#         cursor, timestamps_by_sensor, col2, selected_units["Wind"], selected_unit_modifiers["Wind"],
#         "Avg Wind Speed"
#     )
#
#     # Average Wind Degree/Direction
#     wind_dir_query = (f'SELECT {generic_units["Wind_Direction"]} '
#                       f'FROM weather_data WHERE sensor_id = ? AND timestamp = ?')
#     wind_dir_results = []
#     for sid, mt in timestamps_by_sensor:
#         cursor.execute(wind_dir_query, (sid, mt))
#         wind_dir_results.append(cursor.fetchone()[0])
#
#     get_avg_metric(
#         cursor, timestamps_by_sensor, col3, generic_units["Wind_Degree"],
...
#     get_avg_metric(
#         cursor, timestamps_by_sensor, col4, selected_units["Pressure"], selected_unit_modifiers["Pressure"],
#         "Avg Air Pressure"
#     )
#
#     # Average Precipitation
#     get_avg_metric(
#         cursor, timestamps_by_sensor, col5, selected_units["Precipitation"],
#         selected_unit_modifiers["Precipitation"], "Avg Precipitation"
#     )
#
#     # Average Humidity
#     get_avg_metric(
#         cursor, timestamps_by_sensor, col6, generic_units["Humidity"], generic_unit_modifiers["Humidity"],
#         "Avg Humidity"
#     )
#
#     # Average UV Index Score
#     get_avg_metric(
#         cursor, timestamps_by_sensor, col8, generic_units["UV"], generic_unit_modifiers["UV"],
#         "Avg UV Index Score"
#     )
#
#
...
#     get_hist_section(
#         conn, start_date, end_date, just_id_choices, id_locale_map, selected_units['Temperature'],
...
#     get_hist_section(
#         conn, start_date, end_date, just_id_choices, id_locale_map, selected_units['Wind'],
...
#
#     if len(just_id_choices) == 1:
#         query = (f"SELECT sensor_id, timestamp, {generic_units['Wind_Direction']} FROM weather_data "
#                  f"WHERE sensor_id = {just_id_choices[0]}")
#     else:
#         query = (f"SELECT sensor_id, timestamp, {generic_units['Wind_Direction']} FROM weather_data "
#                  f"WHERE sensor_id IN {tuple(just_id_choices)}")
#     wind_dir_df = pd.read_sql_query(query, conn)
#
#     wind_dir_df['timestamp'] = pd.to_datetime(wind_dir_df['timestamp'])
#     wind_dir_df = wind_dir_df[(wind_dir_df['timestamp'] >= start_date) & (wind_dir_df['timestamp'] <= end_date)]
#     wind_dir_df = wind_dir_df[['timestamp', generic_units['Wind_Direction']]]
#     wind_dir_df_groups = wind_dir_df.groupby(['timestamp', generic_units['Wind_Direction']])
...
#     get_hist_section(
#         conn, start_date, end_date, just_id_choices, id_locale_map, selected_units['Pressure'],
...
#     get_hist_section(
#         conn, start_date, end_date, just_id_choices, id_locale_map, selected_units['Precipitation'],
...
#     get_hist_section(
#         conn, start_date, end_date, just_id_choices, id_locale_map, generic_units['Humidity'],
...
#     get_hist_section(
#         conn, start_date, end_date, just_id_choices, id_locale_map, generic_units['UV'], generic_unit_modifiers['UV'],
...
def create_web_page():
    ...
    ENV_DB_PASSWORD = open("/run/secrets/iot_temp_web_view_password").read().strip()
    ENV_DB_PORT = 5432
    ...
    # Tabs
    tab1, tab2, tab3 = st.tabs(["Sensor Info", "Latest Weather Info", "Historical Trends Info"])
